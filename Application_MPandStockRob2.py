...
## Specify SVAR
name = "Application_MPandStock_m2"
...
if __name__ == '__main__':
    ...
    tempindex[np.logical_and(year==1984,month==1)] = False
    ...
    data_block2 = np.array([logsp500[tempindex],
                                  gs1[tempindex],
                                  ebpnew[tempindex]])
    cummulative = [False, False, False, False, False ]
    varnames = np.array(['y', '\pi',  's', 'i', 'fin'])
    shocknames = np.array(['y', '\pi',  'info', 'mp', 'fin'])
    # SVAR Data
    ...
    if estimator_CUE_Blocks:
        ...
        fig = SVAR.SVARbasics.plot_IRF(out_irf, irf_bootstrap=out_boootstrap_irf, alphas=alphas, shocks=np.arange(2,5),
                                       responses=[], shocknames=shocknames,
                                       responsnames=varnames, cummulative=cummulative, sym=sym)
        # Generate and save Figure 3 in the appendix
        fig_name =  'FiguresAndTables\FigureAppendixC5.pdf'
        fig.savefig(fig_name, format='pdf', dpi=300)
        eps_filename =  'FiguresAndTables\FigureAppendixC5.eps'
